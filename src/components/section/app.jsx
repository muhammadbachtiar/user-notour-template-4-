...
export default function App() {
  ...
  return (
        <section className="w-full flex justify-center">
           <div className='flex rounded-md mb-2 max-w-11/12 w-full justify-center bg-[#850000] pb-1 md:py-2 fixed bottom-0 z-10 dark:bg-gray-700 dark:border-gray-600 my-0 md:my-4 md:static md:grid md:grid-cols-8 md:gap-2 md:bg-transparent md:border-0'>
                ...
                <div style={{ WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', msOverflowStyle: 'none' }}  className="max-w-screen overflow-x-auto flex flex-row md:flex-wrap col-span-8 min-h-14 md:justify-center items-center w-full h-full font-medium gap-y-4">
                        {
                                    ...
                                    return <div key={index} className={`w-full md:w-fit flex items-center ${index !== data.value.length - 1 ? 'border-r border-gray-300' : ''}`}>
                                                {
                                                    item.link.startsWith("http") ? (
                                                        <a href={item.link} target='blank' key={index} rel="noopener noreferrer" className="group min-w-16 w-full md:min-w-40 md:w-fit inline-flex flex-col items-center justify-center focus:ring-0 focus:bg-none md:p-0 md:col-span-2 md:rounded-lg md:justify-items-center md:bg-white dark:hover:bg-gray-800 md:focus:ring-2 md:focus:ring-gray-200 md:focus:bg-gray-100">
                                                                <div className="relative py-2 overflow-hidden rounded-sm w-full flex justify-center group">
                                                                    <IconComponent className='w-6 h-6 object-cover transform group-hover:scale-110 group-focus:scale-125 md:group-focus:scale-110 transition duration-300 ease-in-out text-white md:w-20 md:h-20 md:mb-2 md:selft-center md:text-[#2B2024] group-hover:text-[#850000] group-focus:text-white/60 dark:text-gray-400 dark:group-hover:text-blue-500'/> 
                                                                </div>
                                                                <span className="mt-1 md:mt-0 text-sm md:max-w-32 line-clamp-2 font-normal underline-offset-2 text-white md:text-md text-center md:font-bold md:tracking-tight md:text-[#2B2024] group-focus:text-white/60 dark:text-gray-400 dark:group-hover:text-blue-500">{item.title}</span>
                                                            </a>
                                                        ) : (
                                                            <Link href={item.link} key={index} className="group min-w-16 w-full md:min-w-40 md:w-fit inline-flex flex-col items-center justify-center focus:ring-0 focus:bg-none md:p-0 md:col-span-2 md:rounded-lg md:justify-items-center md:bg-white dark:hover:bg-[#2B2024] md:focus:ring-2 md:focus:ring-gray-200 md:focus:bg-gray-100">
                                                                <div className="relative py-2 overflow-hidden rounded-sm w-full flex justify-center group">
                                                                    <IconComponent className='w-6 h-6 object-cover transform group-hover:scale-110 group-focus:scale-125 md:group-focus:scale-110 transition duration-300 ease-in-out text-white md:w-20 md:h-20 md:mb-2 md:selft-center md:text-[#2B2024] group-hover:text-[#850000] group-focus:text-white/60 dark:text-gray-400 dark:group-hover:text-blue-500'/> 
                                                                </div>
                                                                <span className="mt-1 md:mt-0 text-sm md:max-w-32 line-clamp-2 font-normal underline-offset-2 text-white md:text-md text-center md:font-bold md:tracking-tight md:text-[#2B2024] group-focus:text-white/60 dark:text-gray-400 dark:group-hover:text-blue-500">{item.title}</span>
